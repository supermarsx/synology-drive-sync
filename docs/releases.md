...
The Linux archives target GNU libc and conservatively require glibc 2.35 or newer. They are built on the latest stable Ubuntu 24.04 x86-64 and ARM64 hosted runners with a checksum-pinned Zig 0.16.0 linker and an explicit glibc 2.35 target. The workflow then proves that each produced binary references no GLIBC symbol newer than 2.35; its exact minimum may be lower. This separates the maintained runner image from the published compatibility floor while preserving the host's normal resolver/NSS behavior. The Linux vault backend talks to freedesktop Secret Service over D-Bus using the Rust `zbus` stack; it does not dynamically link `libsecret` or `libdbus`, and the release workflow checks that boundary. A usable vault still requires an active user D-Bus session and an unlocked Secret Service collection.
...
> `authenticate.cgi`. Release 26.21 and later accept that ownership while retaining root-only
> ancestor and symlink validation. Never modify DSM-owned helper files to work around 26.20.
...
The repository source `third_party_licenses.html` is generated by hash-pinned `cargo-about` 0.9.2
and published as the compatibility-stable `THIRD_PARTY_LICENSES.html` release asset. It covers
...
- It also stamps the target triple, cargo profile, and commit, which appear in the startup log banner and the target diagnostic report. The commit is read from `SDSYNC_BUILD_GIT_SHA` when set, otherwise from `git rev-parse`, and falls back to `unknown` for a source archive with no repository. Set `SDSYNC_BUILD_GIT_SHA` in any pipeline that builds outside a checkout so released binaries identify their own commit.
